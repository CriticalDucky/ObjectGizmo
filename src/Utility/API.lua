...
local function generateAPI()
	...
	if TESTING_LOCALLY then
		...
		if not requestSuccess then -- HttpService not on
			warn(
				"HttpService.HttpEnabled is not true. HttpService is required briefly upon getting the Roblox API."
			)
			warn("Please enable HttpEnabled and retry.")
			return false
		end
		if not requestResponse.Success then
			...
			--warn("Please retry. If this persists, contact Dekkonot.")
			return false
		end
		jsonSuccess, apiJson = pcall(
			HttpService.JSONDecode,
	...
	local function getProperties(class, pluginContext)
		if pluginContext == nil then
			error("pluginContext must be passed to API.getProperties")
		end
